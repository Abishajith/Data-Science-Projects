...
class ModelPusher:
    ...
    def initiate_model_pusher(self,)->ModelPusherArtifact:
        try:
            ...
            #Creating model pusher dir to save model
            model_file_path = self.model_pusher_config.model_file_path
            os.makedirs(os.path.dirname(model_file_path),exist_ok=True)  ##create the folder 
            shutil.copy(src=trained_model_path, dst=model_file_path)     ## save the model

            #saved model dir
            saved_model_path = self.model_pusher_config.saved_model_path
            os.makedirs(os.path.dirname(saved_model_path),exist_ok=True) ##create the folder 
            shutil.copy(src=trained_model_path, dst=saved_model_path)    ## save the model
